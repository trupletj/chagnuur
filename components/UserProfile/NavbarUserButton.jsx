  ...
  return (
    <div className="relative flex space-x-1">
      <div
        ...
        className="  h-9 rounded-full bg-red-400 px-1 text-white font-bold flex justify-center items-center cursor-pointer "
      >
        <div className="relative h-7 w-7 rounded-full bg-amber-300 mr-2 flex items-center justify-center overflow-hidden">
      ...
      {show && (
        <div
          ref={wrapperRef}
          className="absolute flex flex-col items-center  top-[40px] min-w-[225px] space-y-2 right-0 bg-white py-4  rounded shadow-[0_6px_32px_rgba(23,29,41,0.1)] text-black "
        >
          <div className="flex flex-col px-2">
            <div className="w-12 h-12  bg-amber-300 rounded-full mx-auto flex items-center justify-center relative text-white">
              {session.user?.name[0].toUpperCase()}
            </div>
            <h1 className="font-bold text-center text-xl">
              {session.user?.name} {session.user?.last_name}
            </h1>
            <h1 className="text-xs text-center">{session.user?.email}</h1>
          </div>
          <div className="w-full px-2 space-y-1">
            <button className="flex items-center space-x-1 p-2 rounded-3xl hover:bg-slate-100 w-full ">
            ...
            <button
              className="flex items-center space-x-1 p-2 rounded-3xl hover:bg-slate-100 w-full "
              onClick={() => signOut()}
            >
              <div className="w-4 h-4">
                <svg
